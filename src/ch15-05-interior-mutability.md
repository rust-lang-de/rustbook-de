...
einzige Eigentümerschaft für die darin enthaltenen Daten. Was unterscheidet
...
* Referenzen müssen immer gültig sein.
    ...
    Die Überprüfung der Ausleihregeln zur Kompilierungszeit hat den Vorteil, dass
    Fehler früher im Entwicklungsprozess erkannt werden und die Laufzeitleistung
    ...
    Ausleihregeln zur Kompilierungszeit zu überprüfen. Aus diesem Grund ist dies die
    ...
    Überprüfung zur Kompilierungszeit nicht zulässig sind. Die statistische Analyse
    ist wie der Rust-Compiler von Natur aus konservativ. Einige Eigenschaften von
    Programmcode lassen sich durch Analyse des Programmcodes nicht erkennen: Das
    ...
    Ähnlich wie `Rc<T>` ist `RefCell<T>` nur für die Verwendung in einzelsträngingen
    (single-threaded) Szenarien vorgesehen und gibt einen Fehler bei der
    Kompilierung aus, wenn man versucht, ihn in einen mehrsträngigen Kontext zu
...
* `Rc<T>` aktiviert mehrere Eigentümer derselben Daten. Mit `Box<T>` und
    `RefCell<T>` haben Daten nur einen Eigentümer.
* `Box<T>` ermöglicht unveränderliches oder veränderliches Ausleihen, das zur
    Zeit des Kompilierens überprüft wird. `Rc<T>` erlaubt nur unveränderliches
    Ausleihen, das zur Zeit des Kompilierens geprüft wird und `RefCell<T>`
...
```
Es gibt jedoch Situationen, in denen es nützlich wäre, wenn sich ein Wert in
...
die Fähigkeit Fähigkeit zur inneren Veränderlichkeit zu erhalten, allerdings
umgeht `RefCell<T>` die Ausleihregeln nicht vollständig: Der Ausleihprüfer im
...
verwenden können, um den unveränderlichen Lan-Wert zu ändern und herauszufinden,
...
Mechanismus zum Senden der Nachrichten bereitstellen: Die Anwendung einfügen,
eine E-Mail senden, eine Textnachricht senden oder etwas anderes. Die Bibliothek
muss dieses Detail nicht kennen. Alles, was es braucht, ist etwas, das eine von
uns bereitgestellte Eigenschaft namens `Messenger` implementiert. Codeblock
...
ein Wert an einem Maximalwert liegt und um zu warnen wenn der Wert auf
bestimmten Ebenen liegt</span>
...
Text der Nachricht enthält. Dies ist die Schnittstelle, die unser Scheinobjekt
...
Scheinobjekt die erwarteten Nachrichten enthält Codeblock 15-21 zeigt den
Versuch, ein Mock-Objekt zu implementieren, um genau das zu tun, aber der
Ausleihprüfer erlaubt dies nicht:
...
implementieren, der vom Ausleihprüfer nicht erlaubt wird</span>

Dieser Testcode definiert ein `MockMessenger`-Struktur mit einem
...
`MockMessenger` geben können. Bei der Definition der Methode `send` nehmen wir
...
die `send`-Methode einen unveränderlichen Verweis auf `self` verwendet. Wir
...
der Merkmalsdefinition von `Messenger` übereinstimmt (Du kannst es versuchen und
die Fehlermeldung lesen die dabei ausgegeben wird).
...
Wir rufen `bor_mut` auf der `RefCell<Vec<String>>` in `self.sent_messages` auf,
um eine veränderliche Referenz auf den Wert in der `RefCell<Vec<String>>` zu
erhalten, der der Vektor ist. Dann können wir die veränderliche Referenz auf den
Vektor `push` aufrufen, um die während des Tests gesendeten Nachrichten zu
verfolgen.
...
Nachdem du nun gesehen hast, wie du `RefCell<T>` verwenden, wollen wir uns mit
...
Methode `borrow` gibt den intelligenten Zeigertyp `RefMut<T>` zurück. Beide
Typen implementieren `Deref`, sodass wir sie wie reguläre Referenzen behandeln
können.

Der `RefCell<T>` verfolgt, wie viele intelligente Zeiger `Ref<T>` und `RefMut<T>`
derzeit aktiv sind. Jedes Mal, wenn wir `borrow` aufrufen, erhöht die `RefCell<T>`
die Anzahl der aktiven unveränderlichen Ausleihen. Wenn ein `Ref<T>`-Wert
außerhalb des Gültigkeitsbereich liegt, sinkt die Anzahl der unveränderlichen
Ausleihen um eins. Genau wie bei den Ausleihregeln zur Kompilierungszeit können
wir mit `RefCell<T>` zu jedem Zeitpunkt viele unveränderliche Ausleihen oder eine
veränderliche Referenz haben.

Wenn wir versuchen, diese Regeln zu verletzen, anstatt wie bei Referenzen einen
Kompilierfehler zu erhalten, gerät die Implementierung von `RefCell<T>` zur
Laufzeit in Panik (panic). Codeblock 15-23 zeigt eine Modifikation der
Implementierung von `send` in Codeblock 15-22.Wir versuchen absichtlich, zwei
veränderliche Ausleihen zu erstellen, die für denselben Bereich aktiv sind, um
zu veranschaulichen, dass `RefCell<T>` uns daran hindert, dies zur Laufzeit zu
tun.
...
selben Gültigkeitsbereich, um zu sehen, dass `RefCell<T>` in Panik gerät</span>
...
```
Beachten Sie, dass ser Programmcode mit der Meldung `already borrowed:
BorrowMutError` in Panik gerät. Auf diese Weise behandelt `RefCell<T>` zur
Laufzeit Verstöße gegen die Ausleihregel.

Das Abfangen von Ausleihfehlern zur Laufzeit anstelle der Kompilierungszeit
...
Beeinträchtigung der Laufzeitleistung verursachen, da die Ausleihen zur Laufzeit
und nicht zur Kompilierungszeit nachverfolgt werden. Die Verwendung von
...
erstellt haben. Fügen wir `RefCell<T>` hinzu, um sie Werte in den Listen ändern
...
(siehe Abschnitt[“Where’s the `->` Operator?”][wheres-the---operator]), um den
...
Daten-Wettlaufsituationen, und manchmal lohnt es sich, ein wenig Geschwindigkeit
für diese Flexibilität in unseren Datenstrukturen einzutauschen.
...
der Wert nicht auf den inneren Wert verweist, sondern in die `Cell<T>` und aus
diese heraus kopiert wird. Es gibt auch `Mutex<T>`, das eine innere
