...
`Config`-Struktur indem wir in den Anteilstyp indexierten und die Werte klonten
...
Zu diesem Zeitpunkt, sagten wir, dass man sich keine Gedanken wegen den
ineffizienten `clone`-Aufrufen machen soll, da sie zu einem späteren Zeitpunkt
...
`new`-Funktion so ändern das sie die Eigentümerschaft eines Iterators als
...
Öffne die Datei *src/main.rs* deines E/A-Projekts, es sollte so aussehen:
...
#
```
Wir werden den Anfang der Hauptfunktion `main` von Codeblock 12-24 in den 
...
Die `env::arg`-Funktion gibt einen Iterator aus! Anstatt die Werte des Iterators
...
Typ des zurückgegebenen Iterators `std::env::Args` ist. Wir haben die Signatur
...
Schlüsselwort `mut` in die Spezifikation des Parameter `args` eintragen, um es
...
die Iterator Methode zu verwenden</span>
...
zum nächsten Wert zu gelangen und den ersten Rückgabewert überspringen. Als
...
Wir können diesen Programmcode durch die Verwendung von Iteratorenadapter
...
machen. Das Entfernen des Status veränderlich kann uns eventuell zukünftige
...
#}
```
<span class="caption">Codeblock 13-29: Verwendung von Iteratorenadapter-Methoden
...
um nur die Zeilen beizubehalten, die `line.contains(query)` `true` zurückgibt.
Wir sammeln dann die passenden Zeilen mit `collect` in einen anderen Vektor.
Viel einfacher! Nimm die gleiche Änderung vor, um Iteratormethoden auch in
Funktion `search_case_insensitive` zu nutzen.

Die nächste logische Frage wäre, welchen Stil du in deinen eigenem Programmcode
wählen solltest und warum. Die ursprüngliche Implementierung im Codeblock 13-28
oder die Version die Iteratoren verwendet, Codeblock 13-29. Die meisten
...
verschiedenen Iteratorenadapter und deren Funktionsweise hast, können Iteratoren 
...
abstrahiert und die einzigartigen Konzepte, wie z.B die Filterbedingung die
